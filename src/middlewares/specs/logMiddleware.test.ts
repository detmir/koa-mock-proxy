...
describe('MemoryLogStorage', () => {
  ...
  it('Should shrink after increase limit', () => {
    const fakeMocks = [1,2,3,4].map(number => createMockContext({ url: `url${number}` }));
    ...
    expect(storage.getItems().length).toEqual(limit)
    expect(storage.getItems().map(item => item.url)).toMatchObject(fakeMocks.slice(1).map(ctx => ctx.url))
  });
})
